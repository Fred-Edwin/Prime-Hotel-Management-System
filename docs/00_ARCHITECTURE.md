...
- **Raw ingredients** (flour, sugar, cooking oil, ...) are received from suppliers and consumed in cooking. Tracked in their own tables (`ingredients`, `ingredient_entries`), logged daily by the store manager, restaurant-location-scoped only (canteen never sees ingredient data — it has no ingredient stock of its own).
