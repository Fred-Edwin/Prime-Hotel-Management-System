...
**Local dev can point at either a local Docker Supabase stack (`npx supabase start`) or a hosted free-tier dev project** — both are "Local dev" in the table above, the choice is just about what's practical on the machine at hand. As of 2026-07-19, this repo's `.env.local` points at a hosted free-tier project (`prosper-hotel-dev`, ref `fbowdsdyccpsumcxcuti`, org "Lobster Technologies") because the local Docker image pull (~2.5GB) was impractical on a slow connection. All migrations are applied there via `supabase link` + `supabase db push`. **Before assuming setup is needed, check whether `.env.local` already exists and has real values** — don't blindly recreate a project or re-run `supabase start`. If `.env.local` is missing or empty, either run `npx supabase start` (needs Docker Desktop with working WSL integration) or create a new hosted project via `npx supabase projects create` + `supabase link` + `supabase db push`, per whichever is practical that session.
