...
**Explicitly not built:** an "undo" for any of these deletes (matching the purchase-delete precedent's own "deliberately narrow, not a general undo" framing); a delete path for `orders` themselves (only the per-order *payment* is deletable, not the order/line-items — that would need to unwind stock deduction too, out of scope for this request); a delete path for `expenses` beyond what already existed (`expenses_delete_admin_only` predates this work and needed no change, since expenses have nothing to unwind either).
