...
The Item Ledger's **ingredients** section (`/dashboard/ledger`, `public.dashboard_ingredient_ledger()`) mirrors this with its own `cost_value = quantity_used * buying_price_snapshot` — the cash value of ingredient actually consumed in cooking that day, distinct from `closing_stock_value` (value of what's left) and `wastage_value` (value of what spoiled). Unlike `stock_entries.cost_value`, this is **not** a stored column on `ingredient_entries` — `quantity_used` and `buying_price_snapshot` are both already stored, so the RPC derives `cost_value` at read time rather than duplicating it (added 2026-07-23, client feedback — `20260723210000_ingredient_ledger_cost_value.sql`).
...
**Cross-location cascade.** A `canteen_supplied` item's restaurant `sent_out` feeds the canteen's `added_stock` via `canteen_supplied_total()` (§3.1). So `recompute_stock_entry_cascade()` also detects when the edited row is `restaurant` + `canteen_supplied`, and cascades from there. **As of the 2026-07-20 daily-cadence conversion**, this is a same-day 1:1 check, not a week-range loop. **Before the conversion**, this looped over every canteen week whose `[entry_date, entry_date+6]` range overlapped the edited date, re-deriving each from a week-range `canteen_supplied_total()` call — replaced because once canteen is daily there's no week window left to loop over.

**Bug found and fixed 2026-07-24 (client report — "store manager has been sending to canteen the whole week but the canteen side isn't reflecting it"):** both this cascade and the live restaurant write paths (`save_stock_entry()`, `save_stock_entry_store_manager_fields()`) previously only *updated* a canteen row if one already existed for that item/date — they never created one. A canteen `stock_entries` row only ever came into existence when canteen staff (Anne) independently saved something for that exact item on `/entry` that same day, via `save_stock_entry_canteen_field()` — the only writer that knew how to insert-or-update a canteen row. If she never touched a given item that day (common for stationery/retail lines canteen stocks but doesn't sell/log daily — Blue Forms, Photocopy, Envelops in the reported data), no row existed, so `added_stock` had nowhere to persist — even though the restaurant had genuinely sent stock. Anne's own `/entry` screen masked this in the moment (`GET /api/stock-entries` always live-computes the same-day total for display via `canteen_supplied_totals_batch()`, regardless of whether a row exists), but the admin Item Ledger reads the *stored* column (`dashboard_item_ledger()`, a plain `select` off `stock_entries`, no live fallback) — so any item/day Anne hadn't separately touched simply didn't appear there at all, which is what the client was seeing.

**Fix** (`20260724150000_canteen_supplied_row_autocreate.sql`): every restaurant `sent_out` writer, and the cascade above, now call `save_stock_entry_canteen_field()` as a mirror-sync after their own write succeeds — reusing its existing insert-or-update logic rather than duplicating it — so the canteen row for that item/date is created (not just updated) the moment the restaurant sends stock, regardless of whether canteen staff have touched it. The live-path calls (`save_stock_entry()`/`save_stock_entry_store_manager_fields()`) wrap this in a non-fatal exception handler: since `added_stock` here only ever increases or holds steady, the sync can only raise `oversell` if canteen's own recorded consumption already exceeded the true total *before* this fix existed (a pre-existing, previously-masked data problem) — that must not block the store manager's own save, which she has no way to fix from her screen, so it logs a Postgres warning instead of rolling back. The cascade's call is left unwrapped, since an admin editing historically is exactly who should see that conflict surface (existing `PATCH /api/dashboard/ledger/entry` error path: "Entry saved, but recalculating later entries failed..."). `recompute_stock_entry_cascade()` gained a `p_created_by` parameter (nullable, defaults to `auth.uid()`) since it can now `insert`, not just `update`, and `stock_entries.created_by` is `not null`.

Historical gap rows from before this fix (any canteen_supplied item/date where the restaurant's `sent_out` has no matching canteen row) are **not** auto-backfilled by this migration — see that migration's own trailing comment / the human-run backfill SQL provided alongside it for closing the gap on already-affected dates.
...
**Bug found and fixed 2026-07-24 (client report — Anne got a 500 saving "Quantity sold" for a `canteen_independent` item despite ample available stock, prod-only reproduction initially):** `save_stock_entry_canteen_field()` had relied on PL/pgSQL's `found` variable across two separate `select into`s ever since its first version. The first select checks whether today's row exists; its `else` branch (no row yet) runs a *second* select to look up the prior day's `closing_stock` for opening-stock carry-forward — which silently overwrites `found` with its own result. Further down, `elsif found then` (intended to mean "did today's row exist?") was actually reading "did a prior day's row exist?". For an item with no row yet today but a real prior day's closing stock (a normal case — e.g. opening stock 7), this incorrectly took the `elsif` branch and read `v_existing.added_stock` off a struct that was never populated — `NULL` — which combined with a `NULL` `p_added_stock_input` (Anne's "Quantity sold" save never sends `added_stock`) produced a `NULL` insert into `stock_entries.added_stock`, a `not null` column: Postgres `23502`, a code `describeSaveError()` doesn't recognize, falling through to a generic 500. Reproduced locally via `curl` as Anne against `prosper-hotel-dev` once tested against the right shape (fresh row + prior stock, not an already-touched row or a zero-history item). **Fix** (`20260724190000_fix_canteen_field_found_clobber.sql`): capture `found` into a dedicated `v_row_found` boolean immediately after the first `select into`, before the second one can clobber it, and use `v_row_found` in place of the live `found` everywhere below. No other logic changed. This `found`-clobber pattern is specific to this function (the only writer with a nested `select into` inside its `else` branch); the other five `stock_entries` writers don't have this shape.
...
## 3.9a Dashboard COGS inflated by a brand-new item/ingredient's own first purchase (client-reported bug, 2026-07-30)

**Client report** (WaPrecious): "After purchasing printing papers, it appears as cost of Sale and yet no sales made. Why?" She'd deleted and re-added an ingredient ("Printing Papers," used as an input to the canteen's separate cyber-café `Printing/Papers` sellable item — the two are unrelated catalog rows that happen to share a name, one an `ingredients` row, one an `items` row of `category: cyber`), logged a purchase of 500 pcs @ KES 1 via `/dashboard/purchases`, with `quantity_used`/`wastage` both correctly 0. The Ledger showed the row exactly right — `received 500, used 0, wasted 0, closing_value 500` — but the dashboard's combined "Cost of goods" (`periodicCogs()`, §3.8) jumped by the full KES 500, for a purchase nothing had consumed yet.

**Root cause, found by tracing `dashboard_ingredient_summary()`'s CTEs directly against the live row (not by reading the code in isolation — the per-row Ledger figures were all individually correct, so the bug had to be in how the dashboard aggregates across rows).** §3.9's `opening` CTE falls back to the `closing` CTE's `closing_stock`/`closing_stock_value` whenever no row exists strictly before `p_from` — i.e. whenever an item/ingredient's earliest-in-range row is also its first-ever row. That fallback is correct for an item that already had real stock on the shelf before its first logged row (§3.9's own docstring: "there is no earlier balance, so its earliest known state is correctly both its opening and current figure" — true when "earliest known state" reflects pre-existing stock). It is **wrong** when the first-ever row's `received`/`added_stock` is itself what created the period's activity: using that same row's post-purchase `closing_stock` as the pre-purchase `opening_stock` hides the purchase from the "opening + added" side of the formula while still subtracting it on the "closing" side, manufacturing unmatched cost equal to the full purchase value. Concretely, for Printing Papers' only row: `opening_stock_value` (wrongly, via the old fallback) = `closing_stock_value` = 500, so `periodicCogs()` computed `500 (opening) + 500 (added) − 500 (closing) = 500` instead of the correct `0 (opening) + 500 (added) − 500 (closing) = 0`.

**The fix should have read the row's own `opening_stock` column, not fabricated one from `closing_stock`.** `save_stock_entry()`/`save_ingredient_entry()` (the write path, unaffected by this bug) already write `opening_stock = 0` for a genuinely first-ever row (line ~674 above). `dashboard_stock_summary()`/`dashboard_ingredient_summary()` just never consulted that column when synthesizing the no-prior-row fallback. `20260730150000_fix_dashboard_opening_stock_new_item_fallback.sql` changes the `closing` CTE to also carry that same row's own `opening_stock`/`opening_stock * buying_price_snapshot`, and changes `opening`'s fallback to read those instead of `closing_stock`/`closing_stock_value`. This is a strict correction, not a behavior change for the case §3.9 actually fixed: for an item whose earliest-in-range row is *not* also its first-ever row (real prior stock existed outside the range), that row's own `opening_stock` already equals the prior day's `closing_stock` by the normal carry-forward rule (§3.1) — reading it directly is exactly as correct as the old `closing_stock` fallback was, and only diverges (correctly) for the genuinely-new-item case.

**Return signature unchanged** — no new/removed columns on either function, no RLS/schema change, no `lib/supabase/types.ts` update needed. Same "extend the per-item-universe CTE structure, don't revert to a flat `group by`" caution from §3.9 applies to any future edit here.

**Not yet independently reproduced against `dashboard_stock_summary()`'s own live data** (the items-side sibling function, same bug pattern, same migration) — only the ingredients side was directly observed failing via the client's report. The fix migration corrects both functions identically since they share the exact same fallback bug by construction, but if a similar "brand-new item purchase inflates COGS" report comes in on the items side, it's the same root cause, already fixed by this same migration, not a new investigation.
...
## 3.16 Deleting a Ledger row (client feedback, 2026-07-27: "add delete options at Item Ledger... I have double entries")

**The request, and what it actually pointed at.** WaPrecious asked for a delete option on the Item Ledger, citing "double entries." `stock_entries` itself can't hold a literal duplicate — `unique(item_id, location, entry_date)` means a re-save always updates the same row — but `staff_meal_entries`/`complimentary_meal_entries`/`stock_adjustment_entries` (§3.5, §3.10, §3.12) have **no unique constraint at all** and are admin-writable straight from the Ledger's edit modal, so a double-tap or a resubmitted claim genuinely does land as two separate rows there. Confirmed with the human that the request covers **every table on this screen**, not just the consumption rows — the main Item Ledger table and the Ingredients table too — with a clear before-you-commit warning whenever a delete has a cascading effect on later rows.

**Shape: three purpose-built reversal functions, following the `ingredient_purchases`/`canteen_stock_purchases` "Deleting a purchase" precedent (§3.2) exactly** — narrow, cascade-aware, admin-only. Not a generic delete primitive on any of these tables.

1. **Non-Sales Stock Consumption rows** (`20260727090000_consumption_entry_delete.sql`) — `delete_staff_meal_entry()` / `delete_complimentary_meal_entry()` / `delete_stock_adjustment_entry()`, each taking just the claim row's own `id`. Deletes the row, then calls `recompute_stock_entry_cascade()` (the same top-level cascade entry point the admin ledger-edit route already uses, §3.4) for that claim's item/location/date — sufficient because none of these three totals are stored redundantly anywhere: every `stock_entries` writer re-sums `staff_meals_total()`/`complimentary_meals_total()`/`stock_adjustments_total()` live, every time it runs. Same atomic-rollback-on-downstream-oversell guarantee as every other cascade in this schema. **Wastage itself is not covered by these three functions** — it's a `stock_entries`/`ingredient_entries` column (§3.3), not a separate claim row, so there is nothing to delete; "removing" a wastage figure means editing it back to 0 via the existing admin ledger-edit path, unchanged. The Ledger UI disables the delete affordance on a wastage row for exactly this reason (its row has no claim id to target — see `dashboard_stock_consumption_ledger()`'s new `entry_id` column below, `null` on both wastage branches).
2. **A whole `stock_entries`/`ingredient_entries` row** (`20260727100000_stock_entry_delete.sql`) — the heavier case: `delete_stock_entry()`/`delete_ingredient_entry()` remove an entire day's row (e.g. a day accidentally logged against the wrong date). Since `opening_stock` is always carried forward from whichever row is immediately prior in the date chain (not "yesterday" literally — §3.1/§3.4), removing a row changes what "the previous row" is for every later one; both functions re-run the same forward cascade anchored at the next *remaining* row's date, so it re-derives correctly regardless of which row was removed. For a **restaurant** row belonging to a `canteen_supplied` item, this also re-syncs the linked canteen day(s), since `canteen_supplied_total()` sums `sent_out` live and a deleted day's contribution needs to drop out of that sum too. Does **not** touch `orders`/`order_items` for that date — an order is its own permanent record (§6), independent of whatever `stock_entries` row happens to summarize it; if real orders still exist for a deleted date, the row is simply recreated (at zero opening/added/wastage) the next time anything touches that date, same "derived rollup, not authoritative" posture the `canteen_supplied` auto-create path already established.
3. **A payment under Debtors** (`20260727110000_order_payment_delete.sql`) — see §6 below; a plain admin-only DELETE policy, no companion function, since outstanding balance is derived at read time and there is nothing to unwind.

**`dashboard_stock_consumption_ledger()` gained an `entry_id` output column** (`20260727120000_consumption_ledger_entry_id.sql`, same drop-then-recreate requirement as every prior return-shape change to this function, §3.9's documented reason) — `null` for both wastage branches, the real claim row `id` for staff_meal/complimentary_meal/stock_adjustment. This is what the Ledger's Non-Sales Stock Consumption table's new per-row Delete action targets.

**Routes:** `DELETE /api/dashboard/ledger/entry` (extends the existing `PATCH` route's file, same discriminated-union shape as its edit counterpart — `ledgerEntryDeleteSchema`, `lib/validation.ts`) covers all three Ledger-hosted row kinds. Each branch fetches the existing row first for the audit log's "before" snapshot, then delegates the actual removal + recompute entirely to the matching `delete_*()` RPC — never a direct client-side `DELETE FROM`. `DELETE /api/orders/[id]/payments/[paymentId]` is new, admin-only, for the Debtors payment case.

**UI:** the Item Ledger's three tables (main table, Ingredients, Non-Sales Stock Consumption) each gained a delete affordance — `components/ActionMenu` (Edit + destructive Delete) replacing the old plain edit-icon button on the two whole-row tables, matching the Staff screen's own precedent for "2+ row actions, one destructive." A shared confirmation `Modal` reuses the existing cascade-impact preview (`GET .../entry/impact`) the edit flow already built, so the admin sees "this will also recalculate N later entries" (or "nothing else will change") before confirming — satisfying the client's explicit "warn her clearly, then delete and cascade" ask. Payments under `/dashboard/debtors` get a plain inline "Delete" text action per payment row, no confirmation step (matching that screen's existing "Record payment" flow, which also commits immediately) — justified by payment delete having no cascade at all to warn about.

**Audit logging:** every new delete calls `writeAuditLog()` (`stock_entry.admin_delete` / `ingredient_entry.admin_delete` / `{table}.admin_delete` / `order_payment.admin_delete`), recording the deleted row as `changes.before` — the only record of what existed once the row is gone.

**New errcode:** `P0007` (`unknown_entry: ...`) — the row being deleted no longer exists (already removed, or a stale client view). `lib/errors.ts`'s `describeSaveError()` surfaces this as "That entry no longer exists." (404), distinct from `P0005`'s pre-existing "invalid_quantity"/overpayment meanings.

**Explicitly not built (at the time of this section):** an "undo" for any of these deletes (matching the purchase-delete precedent's own "deliberately narrow, not a general undo" framing); a delete path for `expenses` beyond what already existed (`expenses_delete_admin_only` predates this work and needed no change, since expenses have nothing to unwind either). **A whole-`orders` delete was originally scoped out here** (payment-only, per the client's initial request) **but was added the same week — see §3.17.**

**A pre-existing, unrelated data bug was found and fixed during this feature's `curl` verification pass, not caused by it.** §3.15's fix for `recompute_stock_entry_chain()` (`20260724160000_fix_cascade_missing_consumption_subtraction.sql` — the function wasn't subtracting `staff_meals`/`complimentary_meals`/`stock_adjustments` from `closing_stock` when cascading a historical edit forward) had been written, documented as applied, and described in this file as fixed — but was **never actually run** against `prosper-hotel-dev`. The live database was still running the old, buggy function the whole time. Verifying `delete_staff_meal_entry()`'s cascade against a real item (Black Tea, 2026-07-24) surfaced it immediately: `closing_stock` came back as 57 instead of the hand-calculated correct value of 42, a discrepancy that only made sense if `staff_meals`/`complimentary_meals`/`stock_adjustments` were all being read as zero — exactly §3.15's documented bug, still live three days after being "fixed." Confirmed via `select pg_get_functiondef(oid) from pg_proc where proname = 'recompute_stock_entry_chain'` in the SQL Editor: the live function body matched the pre-§3.15 version exactly. The human re-ran `20260724160000_fix_cascade_missing_consumption_subtraction.sql` (unmodified, exactly as it sits in the repo) to apply the fix for real, then a no-op admin edit was used to force Black Tea's 2026-07-24 row to recompute under the corrected formula — confirmed `closing_stock` corrected to 42. **No other rows are known to have been affected** — this is the same "landmine that happened to still be harmless" `20260724160000`'s own header already described, since nothing had triggered a historical-edit cascade against an item/location/date with real staff-meal/complimentary-meal/stock-adjustment claims until this delete feature's testing did. **Lesson for future sessions, extending §3.9/§3.14/§3.15's own recurring theme:** a migration file existing in the repo and being *described* as applied in this doc is not proof it was actually run — if a function's live behavior ever seems to contradict what this doc says was fixed, check the actual function body via `pg_get_functiondef` before assuming the doc is right and the bug is new. This session separately found *why* a migration can silently fail to apply via the SQL Editor despite looking successful: the Editor runs a pasted file as one transaction, so an unguarded `create policy` that collides with an already-partially-applied prior attempt rolls back every statement in the file, including unrelated `create or replace function` statements later in the same paste (see §3.16's own migration files above, which now guard every `create policy` with `drop policy if exists` for exactly this reason).

---

## 3.17 Deleting a whole order (client feedback, 2026-07-28)

**The request.** §3.16/§6's payment-delete addition let admin remove a mistaken payment, but WaPrecious's actual ask under "Delete options required under Debtors" was to delete the debt itself, not just reverse one payment against it — a payment-only delete leaves the order (and its outstanding balance recomputed, not zeroed) in place. Confirmed with the human: this is a genuine whole-**order** delete (line items and all), not a "write off this balance" concept — a distinct, heavier action from §3.16's payment delete, which stays as-is alongside this one.

**Why this needs one more step than §3.16's other deletes — a bug found via `curl` verification, fixed same day (`20260728051000_fix_order_delete_quantity_sold.sql`).** `stock_entries.quantity_sold` is never stored per-order — every ordinary writer (§3.4) re-derives it as `till_quantity_sold + sum(order_items for that item/location/date)` at write time. The **original** version of `delete_order()` assumed `recompute_stock_entry_cascade()`/`recompute_stock_entry_chain()` would do this same re-derivation automatically — it doesn't. That cascade function was built for admin **Ledger edits** (§3.4), where `quantity_sold` is already correct by the time the cascade runs; it only re-derives `opening_stock`/`closing_stock`/`sales_value`/`cost_value`/`closing_stock_value` **from** each row's existing `quantity_sold`/`sent_out`/`wastage` — it never re-sums `order_items` itself, for any caller. Deleting an order's `order_items` and then calling the cascade left `quantity_sold`/`closing_stock`/`sales_value` **stale** — confirmed live: a deleted 20-unit credit order left `quantity_sold=20`/`closing_stock=80`/`sales_value=2000` completely unchanged after the delete. Fixed by having `delete_order()` explicitly recompute and write `quantity_sold` (the same `till_quantity_sold + order_items sum` formula every writer uses) for each affected item **before** calling the cascade, so `closing_stock`/values then derive from the corrected number.

- **`delete_order(p_order_id, p_created_by)`** (`20260728050000_order_delete.sql`, corrected same day by `20260728051000_fix_order_delete_quantity_sold.sql`), `security invoker`:
  1. Captures every distinct `item_id` the order's `order_items` reference into an array **before** deleting anything (once the delete cascades, `order_items` for this order no longer exist to query).
  2. Locks each affected item's stock-entry row (`lock_stock_entry_row`) at the order's own location/date, same up-front-locking discipline every cascade in this schema uses.
  3. Deletes the `orders` row — `order_items`/`order_payments` cascade-delete automatically via their existing `on delete cascade` FKs (§2's schema), no manual cleanup of either.
  4. **For each affected item, explicitly recomputes and writes `quantity_sold`** from `till_quantity_sold + sum(remaining order_items for that item/location/date)`, then calls `recompute_stock_entry_cascade()` to re-derive `closing_stock`/`sales_value`/`cost_value`/`closing_stock_value` from that corrected number. Uses the same cascade function every other delete in §3.16 calls, so the oversell-check machinery is present — but unlike those siblings, **an order delete can never actually trigger it**: removing an order only ever *reduces* demand (`quantity_sold`) for that day, which only ever *increases* `closing_stock` for that day and every day after — an increase in available stock can never cause a later day's demand to exceed supply. Confirmed by direct test (deleting an order whose stock a later day's till sale fully depended on) — the delete succeeded and the later day's row correctly re-derived a *larger* closing stock, not a rejection.
- **Deleting an order with recorded payments is allowed, not blocked** (explicit, confirmed choice — the alternative considered was requiring payments removed first). `order_payments` cascade-deletes along with the order. The confirmation UI's job is to warn how much payment history would be lost, not to gate the action.
- **`orders_delete_admin`** (`for delete using (is_admin())`) — `orders` previously had no delete policy at all (`orders_select_scoped`/`orders_insert_scoped`/`orders_update_admin_only` only). Admin-only, enforced at both the route (`requireAdmin()`) and RLS, same two-layer discipline as every other admin write in this codebase.
- **`DELETE /api/orders/[id]`** (new route) — fetches the existing order (with its `order_items`/`order_payments`) for the audit log's "before" snapshot, then delegates to `delete_order()`; never deletes rows directly via the client.
- **UI (`DebtorsClient.tsx`):** a "Delete order" destructive `Button` next to "Record payment" on each outstanding-order row, opening a confirm `Modal` — states which order/customer/amount will be removed and, when the order has payments, an explicit note ("This order has KES X in recorded payments — deleting the order removes those payment records too"). Unlike the plain-inline, no-confirm payment delete (§3.16 — justified there by having no cascade to warn about), this one does get a confirmation step, since deleting a whole order is a heavier, harder-to-reverse-in-spirit action with a real cascade behind it.
- **Audit logging:** `order.admin_delete`, recording the full order (with items/payments) as `changes.before`.

**Explicitly not built:** any partial-order delete (removing one line item but keeping the rest of the order) — this is whole-order only, matching how `stock_entries`/`ingredient_entries` deletes (§3.16) are also whole-row only, not field-level.
...
**Staff-facing payment recording (correction, post-launch addition 2026-07-28).** `POST /api/orders/[id]/payments` and `record_order_payment()` were never actually admin-gated — any authenticated staff/admin who could see the order (via `order_payments_select_scoped`, itself keyed off `orders_select_scoped`'s own location boundary) could already call this route. Phase 11 simply never built a staff-facing UI for it, only the admin `/dashboard/debtors` screen — client feedback ("allow staff to record debtor payments") closed that gap rather than changing any access rule. Added: `GET /api/orders/debtors` and `GET /api/orders/debtors/[customerId]`, staff-facing equivalents of `GET /api/admin/debtors`/`GET /api/admin/debtors/[customerId]/orders` — both call the exact same `security invoker` functions (`dashboard_debtors()`, a plain `orders` select), but gated with `getActingContext()` instead of `requireAdmin()`, so `orders_select_scoped`'s `is_admin() or location = my_location()` check naturally narrows the result to the caller's own location instead of admin's both-locations view — no new SQL, no new RLS policy, no location filter written by hand in either route. Surfaced as a "Debtors" tab on the staff `/orders` screen (`OrdersClient.tsx`): lookup + drill-in + record payment only, deliberately **no delete affordances** — `orders_delete_admin` and `order_payments_delete_admin` (above) remain admin-only, so whole-order delete and payment delete are still admin-only corrections, not part of this addition.

**Payment delete (correction, post-launch addition 2026-07-27 — see §3.16).** The "no payment reversal/refund route" exclusion below is now **partially superseded**: client feedback ("Delete options required under Debtors for admin") added `order_payments_delete_admin` (`20260727110000_order_payment_delete.sql`, `for delete using (is_admin())`) — `order_payments` previously had no delete policy at all. This is a plain RLS-gated delete with **no companion cleanup function**, unlike every cascade-aware delete in §3.16: outstanding balance is derived entirely at read time (`total_amount - sum(order_payments.amount)`, this section's own judgment call #1 above), so removing a payment row is the whole operation — nothing to unwind, nothing to recompute. `DELETE /api/orders/[id]/payments/[paymentId]`, admin-only, surfaced as a plain inline "Delete" action per payment row on `/dashboard/debtors` (no confirm-modal step, since there is no cascade to warn about). Still no *reversal* concept (a correcting negative entry) — this is a straight delete of the mistaken row, which this feature's derive-at-read-time design makes safe to do directly.
...
### Editing and deleting a history row (follow-up, same day)

**The request.** "Make the history table editable in case she needs to delete or edit anything" — a mistaken save (wrong amount typed, wrong date picked) needed a real correction path from the history table itself, not just from re-visiting the entry form by coincidence.

**Edit needed no new mechanism — it already existed, just wasn't reachable from the history table.** `save_cash_reconciliation()`'s upsert-on-`(location, reconciliation_date)` behavior (above) means "editing" a row is just: select that row's date, change the actual-cash/note, save again — the same re-save path a correction has always used. `CashReconciliationClient.tsx`'s entry form already pre-filled from any existing row for the selected date (built in the original pass, so re-opening a date you'd already saved never showed a blank form). The only gap was a fast way to *get there* from a specific history row. **Fix:** each history row gained an `ActionMenu` (`components/ActionMenu` — the same kebab-menu Edit+Delete pattern the Item Ledger already established, §3.16) with an "Edit" action that sets `entryDate` to that row's date and scrolls the entry card into view — no new route, no new schema, just a shortcut into the existing pre-fill behavior.

**Delete is a genuinely new write path — `DELETE /api/dashboard/cash-reconciliation/[id]`, admin-only.** Same shape as `order_payments`' delete (§6, "Payment delete"): a **plain RLS-gated delete with no companion cleanup function**, since `cash_reconciliations` has no cascade to unwind — nothing else in the schema reads this table (no relationship to `closing_stock`/COGS/`netProfit()`, per this section's own "no stock/profit impact" note above), so removing the row is the entire operation. `cash_reconciliations_admin_all`'s existing `for all` RLS policy already covered delete (no new migration needed for the policy itself); the route's `requireAdmin()` check is the first line of defense, same two-layer discipline as every other admin write in this codebase. Fetches the existing row first for the audit log's "before" snapshot (`cash_reconciliation.admin_delete`), then deletes — never a direct client-side `DELETE FROM`.

**Confirm-modal delete, not a no-confirm inline delete.** Unlike `order_payments`' payment-row delete (one line within a bigger order record, judged low-stakes enough for no confirmation step), a `cash_reconciliations` row *is* the whole record — deleting it removes an entire day's reconciliation outright. This follows the Item Ledger's confirm-modal precedent (§3.16) instead: a `Modal` states which date/location/figures would be removed and that it has no effect on recorded sales/stock/profit, before the admin confirms.

**No schema change.** This is a routes-and-UI-only addition — `cash_reconciliations`' table shape, RLS policy, and `save_cash_reconciliation()`/`dashboard_expected_cash()` are all unchanged from the paragraphs above.

**Explicitly not in scope:** any automatic alert/flag when a variance exceeds some threshold (she asked to be able to look it up, not to be proactively warned — a real future enhancement if she asks for it, not assumed here); any breakdown of *which* sale was missed or double-logged (this feature tells her *that* a discrepancy exists and *how much*, not *why* — diagnosing the specific missed/duplicate entry is still a manual investigation, same as her original Excel-era process, just now with a real number to investigate against instead of a vague feeling that the cash doesn't match); any change to how `stock_entries`/orders/credit sales are recorded or validated; a distinct "edit" form/route separate from the existing entry-form re-save (the follow-up above deliberately reuses it rather than building a second write path).

---

## 3.19 Cost of Goods split into "Cost of Goods Sold" and "Stock Revaluation" (client-reported, 2026-07-30)

**Client report** (WaPrecious, via the human operator): a KES 2,046 "Cost of goods" figure on the dashboard didn't add up against what she'd actually sold. Tracing it (same investigation session as §3.9a) found the figure was real but **blended two genuinely different things**: ~KES 771 was real trading cost (Smokies, sodas, a biscuit line, Ugali Vegetables actually sold/used), and ~KES 1,275 came from her having edited "Printing/Papers"'s catalog `buying_price` from KES 3.50 down to KES 1.00 while 94 units (restaurant) + 416 units (canteen) sat untouched in stock. `periodicCogs()` (§3.8: `Opening Stock Value + Added Stock Value − Closing Stock Value`) has no way to distinguish "stock left because it sold" from "stock's recorded value changed because its price was edited" — both silently move the same three terms. She confirmed she edits buying prices **frequently**, because her real supplier costs fluctuate — this is a recurring source of noise in her COGS/net-profit figures, not a one-off data-entry mistake, so it warranted a permanent split rather than a one-time correction to this period's numbers.

**This is a formula change she previously specified (§3.8), further split — not a reversal of it.** Her original instruction (items' + ingredients' opening/added/closing values summed into one combined figure) is fully preserved; this only decomposes that already-combined figure into two labeled parts. It does **not** touch any stored row — `buying_price_snapshot`, `closing_stock_value`, `opening_stock`, etc. are read exactly as before, for every period (today and every historical range), since this is entirely a read-side reinterpretation of already-correct stored numbers, not a rewrite of history.

**The two figures, confirmed with the human:**

- **Cost of Goods Sold** — the value of what was actually sold/used/wasted/etc. this period, priced consistently. This is the real trading-cost figure, and **this is what now feeds Net Profit** (`Sales − Cost of Goods Sold − Expenses − Asset Purchases`).
- **Stock Revaluation** — how much *existing, unsold* stock's recorded value moved purely because its buying price was edited mid-period, isolated out. Shown as its own clearly-labeled dashboard figure, for transparency — **reporting-only, never netted into profit**, exactly the same treatment this dashboard already gives Wastage / Staff Meals / Complimentary Meals / Stock Adjustments (§3.10) and Asset Losses (§3.18's sibling concept). The two figures are a strict decomposition of the old combined number: `costOfGoodsSold + stockRevaluation` always equals `periodicCogs()`'s old output exactly, to the cent.

**Formula, per item (stock_entries, keyed by item_id+location) or per ingredient (ingredient_entries):**

```
revaluation_value = closing_stock_qty * (closing_unit_price - opening_unit_price)
cost_of_goods_sold = (opening_stock_value + added_stock_value - closing_stock_value) - revaluation_value
```

- `closing_stock_qty` / `closing_unit_price` — the same "latest row at or before `p_to`" lookup §3.9's `closing` CTE already established, just also reading that row's own `buying_price_snapshot` (previously only its stock quantities/values were read).
- `opening_unit_price` — **reuses §3.9's existing `opening_before` CTE verbatim, not a new lookup.** `opening_before` already finds "the latest row strictly before `p_from`" to derive `opening_stock`/`opening_stock_value` (the normal yesterday's-closing-becomes-today's-opening carry-forward rule, §3.1); this change only adds that same row's own `buying_price_snapshot` as `opening_unit_price` alongside the quantity/value it already returned. Verified directly against live data before shipping: Printing/Papers (restaurant) had `buying_price_snapshot = 3.50` on its 2026-07-20 row, and "today"'s `opening_stock_value` (329.00 for 94 units) was already being computed from that same 3.50 — confirming `opening_before` is the correct, already-proven-correct source for this price too, not a parallel concept that could drift from it.
- **Brand-new item/ingredient edge case (no row exists before `p_from` at all):** same case §3.9a already had to solve for opening *quantity*, now solved identically for opening *price*. `opening_unit_price` falls back to `closing_unit_price` (that row's own `buying_price_snapshot`) — the same row, not a different one — which makes `revaluation_value` correctly `0` for a brand-new item's first-ever purchase (nothing existed before today to have been revalued). Falling back to `0` instead would have fabricated a fake price swing on every new item's first purchase — the exact same failure shape §3.9a fixed for opening quantity, now avoided for opening price by construction.
- An item whose unit price never changed intra-period needs no separate "did it change" branch — `closing_unit_price − opening_unit_price` is `0` by construction, so `revaluation_value` washes out to `0` for every unaffected item automatically, and the sum across items is unaffected by how many of them had a price edit.

**Implementation:**
- `dashboard_stock_summary(p_from, p_to)` / `dashboard_ingredient_summary(p_from, p_to)` (`20260730180000_split_cogs_sold_vs_revaluation.sql`) gain one new output column each, `revaluation_value` — the per-item/per-ingredient formula above, summed. Required `DROP FUNCTION` first (return-shape change, §3.9's documented Postgres `42P13` pattern) — CTE bodies copied forward from the current, post-§3.9a live versions (confirmed against the actually-deployed function body per §3.16's documented lesson, not an older snapshot), extended in place rather than flattened, per §3.9's "extend the CTE, don't revert to a flat shape" rule.
- `lib/calculations.ts` — new `splitCogs({ openingStockValue, addedStockValue, closingStockValue, revaluationValue })`, returning `{ costOfGoodsSold, stockRevaluation }`. Pure combining step, same convention as `periodicCogs()`/`netProfit()` — `periodicCogs()` itself is unchanged and still exists (its output is exactly `costOfGoodsSold + stockRevaluation`), but is no longer what's handed to `netProfit()`.
- `app/api/dashboard/summary/route.ts` — `combined`/`byLocation.restaurant`/`byLocation.canteen` each gain `costOfGoodsSold`/`stockRevaluation` fields (alongside the existing `costValue`, kept as the sum of both for anyone still reading it). Every `netProfit()` call site (`combined`, restaurant, canteen) now passes `costOfGoodsSold`, not the raw combined `costValue`. `revaluationValue` is summed items+ingredients before the split, same combining discipline `combinedCostValue` already used for its other three inputs.
- `DashboardClient.tsx` — the hero "Total cost" tile is relabeled **"Cost of goods sold"** (now showing `costOfGoodsSold`, not the old combined figure) with a new **"Stock revaluation"** tile alongside it (shown only when nonzero, `moneySigned()` since a price *increase* is a positive swing and a price *decrease* is negative — neither is inherently "bad," unlike Wastage). The Location Performance Comparison table's "Cost of goods" row is relabeled "Cost of goods sold" (now the real trading-cost figure the Net Profit row beneath it is actually computed from) with a new "Stock revaluation" row directly below it, styled with the same reporting-only visual treatment (`comparisonNegative`) the Non-Sales Stock Consumption table already uses for its own reporting-only rows — a deliberate reuse of that "this row isn't part of the profit sum above it" visual convention, not a claim that revaluation is a loss.
- **Daily trend chart (`dashboard_daily_trend()`) is untouched** — same reasoning as §3.8: periodic COGS (and now its split) only means something over a genuine range, not a single day.

**Carried forward:** `netProfit()`'s `costValue` parameter must always receive `splitCogs().costOfGoodsSold` from here on, never `periodicCogs()`'s raw combined output or the API's `costValue` field directly — any future dashboard change that reads cost must go through the split, not reintroduce the blended figure into net profit. If ingredient wastage entry or any other consumption category is ever added to the "opening/added/closing" formula's inputs (currently out of scope), it must be reflected in `revaluation_value`'s own per-item computation too, not just in `cost_value`'s period sums, or a future price-edit scenario involving that category would silently escape the split.

---

## 3.20 Price-edit warning on Items/Ingredients when stock is on hand (client-agreed change, 2026-07-30, companion to §3.19)

**The problem §3.19 doesn't solve on its own.** §3.19 correctly *labels* a price-edit artifact after the fact, but WaPrecious had no way to know, at the moment she edits a `buying_price`, that doing so with stock still on hand would show up as a Stock Revaluation swing on that day's dashboard — the "why did this number move" surprise is what triggered the original investigation. This is a heads-up, not a calculation change.

**Fix:** `PATCH`-ing an item's or ingredient's `buying_price` (`/items`, `/ingredients`) now warns before save when that item/ingredient currently has nonzero stock on hand — e.g. *"This item has 416 units in stock. Changing the price will show as a one-time Stock Revaluation of approximately KES 1,040 on today's dashboard."* The estimate is `(old_price − new_price) × current_quantity_on_hand`, computed client-side from the same on-hand figure the warning displays — deliberately an approximation labeled "approximately," since the actual `revaluation_value` §3.19 will compute later that day also depends on whether any of that stock also moves (sells/is used) before the day closes.

**On-hand quantity source — a new lightweight per-row lookup, not a new dashboard aggregation function.** Neither `/items` nor `/ingredients` previously had any way to fetch a single item's/ingredient's current stock-on-hand outside the full dashboard aggregation functions (`dashboard_stock_summary()`/`dashboard_ingredient_summary()`, §3.7–§3.9a), which are period-scoped and return every item at once — the wrong shape for "how much of this one item exists right now, while its edit drawer is open." `GET /api/items/[id]/on-hand` / `GET /api/ingredients/[id]/on-hand` (new, admin-only) each return the latest `closing_stock` per location (items: summed across restaurant + canteen, since a single catalog `buying_price` edit applies regardless of which location holds the stock; ingredients: restaurant-only, §3.2, so a single figure) via the same "distinct on item_id/ingredient_id order by entry_date desc" pattern `dashboard_low_stock_items()`/`dashboard_low_stock_ingredients()` already use — a plain query against `stock_entries`/`ingredient_entries`, not a new RPC function, since this is a single-row admin-scoped read with no aggregation-across-items requirement.

**Purely informational — no calculation, RLS, or schema change.** The warning does not block or alter the save; it fires from the existing `PATCH` drawers' change-detection (comparing the form's draft `buying_price` against the record's stored value) before `handleSubmit()` runs. If the admin proceeds, the edit saves exactly as it always has — the *only* new behavior is a heads-up shown first.

**Wording correction, same day, found during human verification.** The warning's first draft ("...will show as a one-time Stock Revaluation of approximately KES X **on today's dashboard**") and `stockRevaluation`'s dashboard tooltip both implied the swing appears as soon as the price is saved. That's wrong, and confusingly so: §3.19's `revaluation_value` is computed entirely from **stored `stock_entries`/`ingredient_entries` snapshot rows** (never live `items`/`ingredients.buying_price`, per this schema's price-snapshot-immutability rule), so editing the catalog price alone changes nothing on the dashboard until the *next* stock entry for that item is actually saved — same-day if staff re-save today's entry after the price edit, otherwise whenever it's next logged. Both strings were corrected to say "once its next stock entry is saved — not immediately" instead of implying same-moment visibility. This is a wording-only fix — no change to `revaluation_value`'s formula, the on-hand lookup, or when the warning itself fires (still at edit time, unconditionally on stock > 0, regardless of when the dashboard will actually reflect it).

---

## 3.21 Bug: Stock Revaluation's sign was inverted on the dashboard (found during production verification, 2026-07-30)

**Found while auditing `prime-hotel-demo`'s live dashboard against raw SQL, same day as §3.19/§3.20 shipped.** A full manual walkthrough of a real item ("Printing/Papers" — the exact item from §3.9a's original client report) confirmed the underlying math was correct end to end: hand-computing `closing_stock_qty × (closing_unit_price − opening_unit_price)` from its raw `stock_entries` rows matched `dashboard_stock_summary()`'s returned `revaluation_value` to the cent, for both restaurant and canteen. Combined across restaurant + canteen + ingredients, the correctly-signed total was **−1,524.31** (a net price *decrease* on stock still on hand). The live dashboard displayed **"+KES 1,524"** — same magnitude, opposite sign.

**Root cause: `DashboardClient.tsx` reused `moneySigned()` for `stockRevaluation`, but that formatter's inversion is specific to `stockAdjustmentValue` and doesn't apply here.** `moneySigned()` was built for §3.10's signed stock-adjustment convention, where a *negative* stored value means a *surplus* (good news, stock found) — so it deliberately flips the sign to display a `+`, so a surplus doesn't misread as a bigger loss. Stock Revaluation has no such inversion: a negative `revaluation_value` means the item's buying price genuinely *dropped* while stock was on hand — the recorded value of that stock really did decrease, and a `-` is the correct, literal display. Reusing `moneySigned()` here (§3.19's own implementation) inverted a real decrease into a displayed increase — the exact opposite of what happened, and exactly the kind of "why did this number move" confusion this whole feature was built to eliminate.

**Fix:** new `moneySignedPlain()` (`DashboardClient.tsx`) — shows the arithmetic sign directly (`-KES X` for negative, `+KES X` for positive, plain `money()` for exactly zero), no inversion. Replaces `moneySigned()` at all three `stockRevaluation` call sites (the hero tile, and both Restaurant/Canteen cells in the Location Performance Comparison table's Stock Revaluation row). `moneySigned()` itself is unchanged and still correctly used for `stockAdjustmentValue` and any total that includes it (§3.10) — this was a wrong-formatter-for-this-figure bug, not a defect in `moneySigned()` itself.

**No backend/SQL change** — `revaluation_value`/`splitCogs()`/`app/api/dashboard/summary/route.ts` were already correct; §3.19's underlying formula was verified correct via this same audit. This was purely a display-layer sign bug, caught before it could mislead the client into thinking a price drop was a price increase (or vice versa) on a live production dashboard.

**Lesson, restated from §3.10's own original design note:** a "signed" formatter's inversion behavior is tied to what a negative number *means* for that specific field, not a generic "handle negative money" utility — copying a signed-display helper to a new figure requires re-checking what its sign inversion actually encodes, not just that it produces a `+`/`-` somewhere.

**Follow-up, same audit session: combined Net Profit vs. Restaurant + Canteen net profit don't sum, and that's correct — but wasn't explained on screen.** The human noticed the hero "Net Profit" tile (-7,102) didn't equal the Location Performance Comparison table's Restaurant (-3,302) + Canteen (-1,201) net-profit rows added together (-4,503) and asked why. Traced by hand: the gap (-2,599, ≈ the difference after rounding) is exactly `businessWideExpenses` (KES 2,600 that day) — a real, correctly-applied deduction from *combined* net profit (§3.10: admin-logged expenses with no location, e.g. rent/salaries) that has never been, and by design never will be, split into either location's own P&L row, since there's no fair way to attribute a business-wide cost to one location. Confirmed asset purchases were KES 0 that day (the other combined-only deduction, §3.18's asset register), ruling out a second contributing factor. **Not a bug** — but nothing on screen said so, which is exactly the same "silently confusing" failure class this whole COGS-split feature was built to eliminate elsewhere.

**Fix:** a footnote (`.comparisonFootnote`, `DashboardClient.tsx`/`dashboard.module.css`) beneath the Location Performance Comparison card, shown only when `businessWideExpenses > 0` (same "avoid a permanent KES 0 line" convention as Asset Purchases/Losses/Total Outstanding elsewhere on this dashboard): *"Restaurant and Canteen net profit above don't include KES X in business-wide expenses (rent, salaries, etc.) — that's subtracted only from the combined Net Profit figure at the top of this page, since it isn't tied to one location."* No calculation change — `combined.netProfit`/`byLocation.*.netProfit` are computed exactly as before; this is purely a missing explanation being added where the human's own confusion arose.
