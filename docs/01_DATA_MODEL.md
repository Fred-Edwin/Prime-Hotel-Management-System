...
## 3.11 Estimated value for zero-buying-price items (client feedback, 2026-07-23)

**The problem.** §3.10 noted that WaPrecious had zeroed `items.buying_price` for most/all ingredient-cooked menu items, at the data level, to avoid double-counting cost between menu-item-level and ingredient-level cost tracking (the same overlap `periodicCogs()`'s doc comment describes — an in-house-cooked item's own `buying_price` and the ingredient cost that produced it both contributing to COGS otherwise). That was and remains the right call, and net profit is correctly unaffected by it (§3.10). But `wastage_value`/`staff_meal_entries.value`/`complimentary_meal_entries.value`/`stock_adjustment_entries.value` are all `quantity * buying_price_snapshot` — with `buying_price_snapshot = 0`, every one of these figures collapses to KES 0 for those items, even though real physical stock is genuinely moving (a wasted plate of chapati, a staff meal, a complimentary soda). WaPrecious wants visibility into how much non-sales stock is being consumed for these items — a real KES estimate, not a bare zero — **without this touching profit calculations in any way.**

**The fix — a parallel, purely additive "estimated value" alongside every real value column, never substituted for it:**

- New table `app_settings` — a single-row (`id boolean primary key default true`, enforced via a `check (id)` constraint) business-wide settings table. First and only field: `estimated_cost_ratio numeric(4,3)` (default `0.600`), admin-editable via `PATCH /api/settings`, readable by any authenticated user via `GET /api/settings` (every write function below needs to read it). RLS: `select` for any logged-in user, `update` admin-only — see `20260723110000_app_settings_estimated_cost_ratio.sql`.
- `public.effective_unit_cost(p_buying_price, p_selling_price)` (`20260723130000_estimated_value_helper_functions.sql`) — the one place the fallback rule is expressed: returns `p_buying_price` unchanged when it's `> 0`, otherwise `p_selling_price * estimated_cost_ratio()`. Mirrored exactly in TypeScript by `lib/calculations.ts`'s `effectiveUnitCost()`, used only for the estimated-value figures below — **never** wired into `buyingPriceSnapshot`, `costValue`, `closingStockValue`, `periodicCogs()`, or `netProfit()`, all of which keep using the real (possibly-zero) buying price exactly as before.
- New columns, each holding `quantity * effective_unit_cost(buying_price, selling_price)` alongside the existing (unchanged) `quantity * buying_price_snapshot` column: `stock_entries.wastage_estimated_value`, `staff_meal_entries.estimated_value`, `complimentary_meal_entries.estimated_value`, `stock_adjustment_entries.estimated_value` (signed, same convention as `stock_adjustment_entries.value` — a shortfall gets a positive estimated cost, a surplus a negative one). See `20260723120000_estimated_value_columns.sql`.
- All six `stock_entries` writer functions (`save_stock_entry`, `save_canteen_stock_entry`, `apply_order_to_stock_entry`, `save_stock_entry_store_manager_fields`, `save_stock_entry_cashier_field`, `save_stock_entry_canteen_field`) and the three per-claim writer functions (`create_staff_meal_entry`, `create_complimentary_meal_entry`, `create_stock_adjustment_entry`) got the same mechanical addition — one `effective_unit_cost()` call, one extra column in each `INSERT`/`ON CONFLICT DO UPDATE` — matching the precedent §3.5/§3.10 already established for adding a term to every writer at once. See `20260723140000_wastage_estimated_value.sql`, `20260723150000_consumption_entries_estimated_value.sql`.
- `dashboard_stock_consumption_ledger()` (the unified tagged-union ledger, §3.10) now returns `estimated_value` alongside `value` for every row. Ingredient wastage (the one branch with no per-row zeroed-price case in practice — ingredients themselves were never zeroed, only the menu items cooked from them) reuses its own `wastage_value` as `estimated_value`, since the two are always equal there. See `20260723160000_consumption_ledger_estimated_value.sql`.
- `dashboard_stock_summary()`, `dashboard_staff_meal_summary()`, `dashboard_complimentary_meal_summary()`, `dashboard_stock_adjustment_summary()` gained the equivalent aggregate `estimated_value`/`wastage_estimated_value` sums, surfaced by `app/api/dashboard/summary/route.ts` as a parallel `estimatedTotal`/`*EstimatedValue` set of fields alongside the existing `stockConsumption` block — same shape, same "never fed into `netProfit()`'s inputs" rule §3.10 already established for the real figures. See `20260723170000_dashboard_summary_estimated_values.sql`.

**UI:** `LedgerClient.tsx`'s Non-Sales Stock Consumption table gained an "Estimated value" column (shown as `—` when it equals the real `Value`, since most rows — anything with a real buying price — will always match); a new "Estimated value settings" button opens a small modal (`GET`/`PATCH /api/settings`) for the admin to view/change `estimated_cost_ratio`. `DashboardClient.tsx`'s comparison table shows a small secondary "est. KES X" line under each cell only when the estimated figure diverges from the real one. Neither the hero "Non-sales stock consumption" `MetricCard` nor `netProfit`/`costValue` anywhere in the dashboard were changed — this is additive display only.

**Explicitly not in scope:** any per-item override of the ratio (confirmed with the human: one global admin-editable rate, not per-item — see the ratio's own ledger-modal help text); retroactively backfilling `wastage_estimated_value`/`estimated_value` on rows written before this migration (they'll show `0`/equal-to-real until next touched — no backfill script, consistent with this schema's general "we don't rewrite history" posture elsewhere); any change to `buying_price` itself (the zeroing WaPrecious already did at the data level is untouched and correct).

**Carried forward:** any future consumption category (if one is ever added alongside wastage/staff meals/complimentary meals/stock adjustments) should get the same paired `value`/`estimated_value` columns and go through `effective_unit_cost()`, not re-derive its own fallback logic. `netProfit()`/`periodicCogs()`'s inputs remain exactly `{ salesValue, costValue, expenses }` / `{ openingStockValue, addedStockValue, closingStockValue }` — no `*_estimated_value` field may ever be added to either.
