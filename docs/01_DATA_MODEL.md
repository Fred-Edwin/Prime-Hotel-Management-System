...
- **`record_canteen_stock_purchase(p_item_id, p_purchase_date, p_quantity, p_unit_cost, p_created_by, p_supplier_note)`** is the single write path, called from `/dashboard/canteen-purchases`:
  ...
  2. Normalizes `p_purchase_date` to that week's Monday server-side — same convention as every other canteen write path (§3.1), never trusted verbatim.
  3. Locks the item/week's `stock_entries` row via `lock_stock_entry_row(item_id, 'canteen', week_start)` — the **same** advisory lock `save_stock_entry_canteen_field()` itself takes, so a purchase and a concurrent autosave from Anne's `/entry` screen touching the same item/week correctly serialize against each other, rather than racing.
  4. Derives quantity-on-hand (this week's `opening_stock + added_stock` if a row already exists, otherwise the prior week's `closing_stock`) and recalculates `items.buying_price` as a weighted average — identical formula to `record_ingredient_purchase()`.
...
**Cross-location cascade.** A `canteen_supplied` item's restaurant `sent_out` feeds the canteen's `added_stock` for that week via `canteen_supplied_total()` (§3.1) — but only when the canteen row is itself (re)saved. So `recompute_stock_entry_cascade()` also detects when the edited row is `restaurant` + `canteen_supplied` and `sent_out` changed, re-derives every canteen week touched by the edited date range from `canteen_supplied_total()`, and cascades *that* forward too, all in the same transaction.
