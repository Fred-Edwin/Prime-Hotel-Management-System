...
import { complimentaryMealSchema } from "@/lib/validation";
import { createServerSupabaseClient } from "@/lib/supabase/server";
import { describeSaveError, serverErrorResponse } from "@/lib/errors";

/**
 * GET /api/complimentary-meals?date=YYYY-MM-DD
 ...
 * already-logged complimentary-meal claims (any staff member's, same
...
export async function GET(request: Request) {
  ...
  const [itemsRes, claimsRes, availableStockRes] = await Promise.all([
    supabase
      ...
      .order("name"),
    supabase
      .from("complimentary_meal_entries")
      .select("*, items(name), users!complimentary_meal_entries_staff_id_fkey(name)")
      .eq("location", user.location)
      .eq("meal_date", date)
      .order("created_at", { ascending: false }),
    supabase.rpc("complimentary_meal_available_stock", { p_location: user.location, p_as_of_date: date }),
  ]);

  if (itemsRes.error) return serverErrorResponse(itemsRes.error, "complimentary-meals");
  if (claimsRes.error) return serverErrorResponse(claimsRes.error, "complimentary-meals");
  if (availableStockRes.error) return serverErrorResponse(availableStockRes.error, "complimentary-meals");
...
/**
 * POST /api/complimentary-meals
 * Logs a single complimentary-meal claim (item + quantity + optional
 * note) for the caller's own location and today's date, attributed to
 * the caller. Mirrors POST /api/staff-meals exactly — value is never
 * accepted from the client, public.create_complimentary_meal_entry()
 * derives it server-side and atomically re-derives the item's
 * stock_entries row (closing_stock, oversell re-check) in the same
 * transaction.
 */
export async function POST(request: Request) {
  ...
  const parsed = complimentaryMealSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const { data, error } = await supabase.rpc("create_complimentary_meal_entry", {
    p_item_id: item_id,
